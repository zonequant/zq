...
#
# settings_files = [
#     Path(__file__).parent / 'settings.yml','.secrets.yaml'
# ]  # 指定绝对路径加载默认配置

settings = Dynaconf(
    envvar_prefix="ZQ",  # 环境变量前缀。设置`EXAMPLE_BLOG_FOO='bar'`，使用`settings.FOO`
    settings_files=['settings.yml','.secrets.yaml'],
    ...
    includes=[os.path.join(sys.prefix, 'etc', 'zq', 'settings.yml','.secrets.yaml')],  # 自定义配置覆盖默认配置
